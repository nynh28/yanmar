...
const { Types, Creators } = createActions({
  setSelectedDashboardMachineStatus: ['status'],
  setDashboardMachineDataTable: ['data'],
  setDashboardFetchTimeData: ['time'],
  setDataMachineDashboard: ['list'],
...
export const TractorUserTypes = Types;
...
export const INITIAL_STATE = Immutable({
  ...
  selectedDashboardMachineStatus: -1,
  dashboardMachineData: [],
  dashboardMachineDataTable : [],
  dashboardMachinePieChartData: {
    normal: 0,
    waiting: 0,
    overdue: 0,
    total: 0
...
const updateDataStackerMachineDashboard = (list) => {
  const dataStacker = groupBy(list, 'dealer_id')
  let dataGroupByDealer = dataStacker
  let label = Object.values(dataStacker).map(i => i[0].dealer_name)
  let series = [
    {
      type: 'bar',
      stack: 'a',
      name: 'Normal',
      color: '#4AA3E7',
      data_raw: Object.values(dataStacker).map(item => item.filter(i => i.maintenance_status == 0).length)
    },
    {
      type: 'bar',
      stack: 'a',
      name: 'Waiting',
      color: '#F4B73F',
      data_raw: Object.values(dataStacker).map(item => item.filter(i => i.maintenance_status == 1).length)
    },
    {
      type: 'bar',
      stack: 'a',
      name: 'Overdue',
      color: '#E45D33',
      data_raw: Object.values(dataStacker).map(item => item.filter(i => i.maintenance_status == 2).length)
    },
  ]
  return {label, series, dataGroupByDealer}
}

export const setDataMachineDashboard = (state, {list}) => {
  let dashboardMachineData = [...list]
  let normal = dashboardMachineData.filter(i => i.maintenance_status == 0)
  let waiting = dashboardMachineData.filter(i => i.maintenance_status == 1)
  let overdue = dashboardMachineData.filter(i => i.maintenance_status == 2)
  let dashboardMachinePieChartData = {
    normal: normal.length,
    waiting: waiting.length,
    overdue: overdue.length,
    total: dashboardMachineData.length
  }
  let dashboardMachineDataTable = []
  if(state.selectedDashboardMachineStatus < 0){
    dashboardMachineDataTable = [...dashboardMachineData]
  }else{
    dashboardMachineDataTable =  [...state.dashboardMachineData].filter(i => i.maintenance_status == state.selectedDashboardMachineStatus)
  }

    
  const {label, series, dataGroupByDealer} = updateDataStackerMachineDashboard(dashboardMachineDataTable)

  const dataChartStackerDealer = {label, series}

  return state.merge({dashboardMachineData, dashboardMachineDataTable, dashboardMachinePieChartData, dataChartStackerDealer, dataGroupByDealer})
}

export const setSelectedDashboardMachineStatus = (state, {status}) => {
  console.log(status)
  if(state.selectedDashboardMachineStatus == status){
    status = -1
  }
  let dashboardMachineDataTable = []
  if(status < 0){
    dashboardMachineDataTable = [...state.dashboardMachineData]
  }else{
    dashboardMachineDataTable =  [...state.dashboardMachineData].filter(i => i.maintenance_status == status)
  }
  let dataChartStackerDealer = updateDataStackerMachineDashboard(dashboardMachineDataTable)
  return state.merge({selectedDashboardMachineStatus: status, dashboardMachineDataTable, dataChartStackerDealer});
}

export const setDashboardFetchTimeData = (state, {time}) => {
  console.log(time)

  return state.merge({dashboardFetchTimeData: time});
}
...
export const setListUser = (state, {listUsers}) => {
  // console.log(state, listUsers)
  return state.merge({listUsers});
}
///////////////////////////////
...
export const reducer = createReducer(INITIAL_STATE, {
  ...
  [Types.SET_DATA_MACHINE_DASHBOARD]: setDataMachineDashboard,
  // [Types.SET_DASHBOARD_FETCH_TIME_DATA]: setDashboardMachineDataTable,
  [Types.SET_SELECTED_DASHBOARD_MACHINE_STATUS]: setSelectedDashboardMachineStatus,
  // [Types.SET_LIST_USER]: setListUser,
  [Types.SET_DASHBOARD_FETCH_TIME_DATA]: setDashboardFetchTimeData,
