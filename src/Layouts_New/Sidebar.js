...
class Sidebar extends Component {
  ...
  componentWillMount() {
    ...
    // let newListMenu = setMenu(get(menuUser, 'userMenus', []), path_now[1])
    this.defaultMenu = setMenu(
      get(menuUser, "userMenus", []),
      dataLogin.userId,
      dataLogin.userLevelId
    );
  ...
  findParentMenuId(id) {
    let userMenus = get(this, "props.menuUser.userMenus", []),
      arr,
      key,
      item,
      pId;
    ...
    key = id;
    while (key !== 0) {
      item = userMenus.find((userMenu) => userMenu.menuId === key);
      if (item) {
  ...
  setLayoutMenu(ele, level, userLevelId) {
    ...
    let { id, name, url, icon, iconUrl, active, subMenu, disable } = ele;
      ...
      >
        <a href={"#/" + url}>
          <div style={STYLEDIVMENU}>
            ...
            {/* {Array.isArray(icon) ? <i className={icon[0]} aria-hidden="true" style={STYLEICON}></i>
          : <SVGIcon name={icon} width={18} height={23} />} */}
          </div>
  ...
  setLayoutSubMenu(ele, level) {
    ...
    return (
      <ul
        id={"ul-" + id}
        name={"sub-lv-" + level}
        className={"nav nav-second-level collapse" + (active ? " in" : "")}
      >
        {subMenu.map((el, ind) => {
