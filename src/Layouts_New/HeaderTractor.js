...
import "./assets/css/customTypes.css";
...
import {
  ENDPOINT_BASE_URL,
  ENDPOINT_BASE_TOPIC_EVENT_2,
} from "../Config/app-config";

import Amplify, { PubSub } from "aws-amplify";
import { AWSIoTProvider } from "@aws-amplify/pubsub/lib/Providers";

const LV_SUB_EVENT = [31, 32, 41, 42];
const EVENT_DEALER_ALERT = [10000, 10004, 10005, 10001, 10006];
    ...
    <li>
      ...
      >
        ...
        >
          <img
            src={arg.src}
            style={{ width: 22, height: 15, marginRight: 10 }}
          />
          {arg.name}{" "}
        </div>
      </a>
...
Amplify.configure({
  Auth: {
    identityPoolId: "ap-southeast-1:f8168cbc-7462-4cab-9338-36ab0df2e73e",
    region: "ap-southeast-1",
    userPoolId: "ap-southeast-1_LqVZJik4R",
    userPoolWebClientId: "2435456789iu654tyrutiyoituryetr111",
  },
});

// #Setup connection to AWS IoT
Amplify.addPluggable(
  new AWSIoTProvider({
    aws_pubsub_region: "ap-southeast-1",
    aws_pubsub_endpoint: `wss://a3eer7xfo36bwl-ats.iot.ap-southeast-1.amazonaws.com/mqtt`,
  })
);
...
class Header extends Component {
  ...
  async componentWillMount() {
    let { stateSignin, isAgreement, topicEvent, dataLogin, custIdByUser } =
      this.props;
    ...
    if (LV_SUB_EVENT.includes(dataLogin.userLevelId)) {
      let custList = [];
      if (custIdByUser.length > 0) custList = custIdByUser;
      else custList = await this.getCustomerByUserId(dataLogin.user_id);

      let topics = [];
      for (let index in custList) {
        // let subEventTopic = PubSub._pluggables[0]._topicObservers.get(this.props.topicEvent)
        if (!TOPIC_EVERT && isAgreement) {
          // if (!TOPIC_EVERT && isAgreement && topicEvent) {
          // Subscribe
          ...
          if (topicConnect === undefined || topicConnect.size === 0) {
            topics.push(
              ENDPOINT_BASE_TOPIC_EVENT_2 + custList[index].int_cust_id
            );
          }
        }
      }
  ...
  subscribe(topics) {
    // TOPIC_EVERT = PubSub.subscribe("TestMessage").subscribe({
    TOPIC_EVERT = PubSub.subscribe(topics).subscribe({
      ...
      next: (data) => {
        ...
        if (
          [31, 32].includes(this.props.dataLogin.userLevelId) &&
          EVENT_DEALER_ALERT.includes(data.value.event_id)
        ) {
          this.props.setToastEvent(data.value);
        } else if ([41, 42].includes(this.props.dataLogin.userLevelId)) {
          this.props.setToastEvent(data.value);
        }
      },
  ...
  testClick = () => {
    if (this.props.status_sidebar && this.props.status_sidebar == "show") {
      ...
      this.props.status_sidebar &&
      this.props.status_sidebar == "hide"
    ) {
    ...
    return (
      <div id="header-fixed" className="row">
        ...
        <nav
          className="navbar"
          role="navigation"
          style={{ marginBottom: 0, height: 55 }}
        ></nav>
        <nav
          className="navbar-static-top navbar-fixed-top"
          role="navigation"
          style={{
            display: "flex",
        ...
        >
          ...
          >
            {this.props.isAgreement && (
              <a
                className="navbar-minimalize minimalize-styl-2 btn btn-primary-collapse"
                id="pace-remix"
            ...
            >
              <div
                ...
                style={{
                  marginBottom: 5,
                  width: this.props.sidebar_width
                    ? this.props.sidebar_width + 50
                    : "100%",
                }}
              >
                <img
              ...
              LV_SUB_EVENT.includes(dataLogin.userLevelId) && (
                <li className={this.state.notifybox} id="notification-li">
                  ...
                  {this.state.notifybox === "dropdown open" && (
                    <HeaderNotification
                      onClickDropdown={(id) => this.onClickDropdown(id)}
                    />
                  )}
                </li>
              )}
              ...
              >
                <li>
                  ...
                  >
                    ...
                    <b>{get(dataLogin, "display_name", "-")}</b>
                  </div>
                </li>
                <li className="divider" />
                <li>
                  <a href="#/Tractor/updateProfile">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getInitialEventData: (update) =>
      dispatch(RealtimeActions.getInitialEventData(update)),
    statusSidebar: (status) => dispatch(VersatileActions.statusSidebar(status)),
    getDataDictionary: (language) =>
      dispatch(VersatileActions.getDataDictionary(language)),
    getMessageCount: () => dispatch(NotificationRedux.getMessageCount()),
    setToastEvent: (toastEvent) =>
      dispatch(NotificationRedux.setToastEvent(toastEvent)),
    setCustomerByUserId: (data) =>
      dispatch(SigninActions.setCustomerByUserId(data)),
    setMessageList: (messageList) =>
      dispatch(NotificationRedux.setMessageList(messageList)),
    setUnreadTotal: (total) =>
      dispatch(NotificationRedux.setUnreadTotal(total)),
    signoutTractor: (redisKey) =>
      dispatch(SigninActions.signoutTractor(redisKey)),
  };
};
