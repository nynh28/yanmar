...
export function* getProfile(api, { }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setProfile(response.data))
  }
  else {
...
export function* getListUser(api, {data}){
  // console.log('data', data)
  const response = yield call(api.getListUser, data)
  // console.log(response)
  if(response.ok){
    yield put(TractorUserActions.setListUser(response.data))
...
export function* getDrop11(api, { }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setDrop11(response.data))
  }
  else {
...
export function* addProfile(api, { data }) {
  ...
  if (response.ok) {
    ...
    yield put(TractorUserActions.setAddProfile(response.data))
    yield put(TractorUserActions.submitStatusVehicle(true, "SUCCESS"))
    // console.log("addProfile_OK", response.ok)
  } else {
    yield put(TractorUserActions.submitStatusVehicle(false, response.data.Error.ErrorSubcode))
...
export function* listVehicle(api, { }) {
  ...
  // success?
  if (response.ok === true) {
    yield put(TractorUserActions.setListVehicleSuccess(response.data.data))
    yield put(TractorUserActions.setListVehicleFail(null))
  } else {
    // yield put(ListTractorUserActions.setListVehicleFail(response.data.message))
    yield put(TractorUserActions.setListVehicleFail(response.problem))
    // yield put(TractorUserActions.setListVehicleSuccess(null))
  }
}
...
export function* addVehicle(api, { data }) {
  ...
  if (response.ok == true) {
    yield put(TractorUserActions.submitStatusVehicle(true, "SUCCESS"))
    yield put(TractorUserActions.setAddVehicleSuccess(response.data))
  } else {
...
export function* vehicleTypeByLaw(api, { data }) {
  ...
  for (let i in data) {
    ...
    if (response.ok == true) {
      yield put(TractorUserActions.setVehicleTypeSuccess(response.data, data[i]))
    }
  ...
  // if (response.ok == true) {
  //   yield put(TractorUserActions.setVehicleTypeSuccess(response.data, data))
  // }
}
...
export function* editVehicle(api, { id, data }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.submitStatusVehicle(true, "SUCCESS"))
    yield put(TractorUserActions.setEditVehicleSuccess1(response.data))
  }
  else {
...
export function* checkVinno(api, { VehicleBrandId, VinNo }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setVinSuccess1(response.data))
...
export function* getInfoVehicle(api, { id }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setInfoVehicle(response.data))
  }
  else {
...
export function* getInfoVehicleExtension(api, { id }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setInfoVehicleExtension(response.data))
    // console.log("SUCCESS", response)
  } else {
    yield put(TractorUserActions.setInfoVehicleExtension(response.data.Error.ErrorSubcode))
...
export function* deleteVehicle(api, { id }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.submitStatusVehicle(true, "customer_130"))
    yield put(TractorUserActions.setdeleteVehicleSuccess())
    console.log("deleteVehicle")
  } else {
    yield put(TractorUserActions.submitStatusVehicle(true, "customer_131"))
  }
}
...
export function* updateVehicle(api, { vehicle }) {
  ...
  if (response.ok) {
    yield put(TractorUserActions.setData(null))
    yield put(TractorUserActions.setListVehicle())
  } else {
