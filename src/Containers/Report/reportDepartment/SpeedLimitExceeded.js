...
class SpeedLimitExceeded extends React.Component {
  constructor(props) {
    super(props)
  ...
  async loaddata() {
    ...
    //console.log(cust_id)
    var api = ENDPOINT_BASE_URL + "fleet/report/dlt/overspeed"
    var object = {
  ...
  async loaddatadetail(id) {
    var api = ENDPOINT_BASE_URL + "fleet/report/dlt/overspeed/" + id
    var start_date = this.props.startDate
    var start_date_y = start_date.split('/')[0]
    var start_date_m = start_date.split('/')[1]
    var start_date_d = start_date.split('/')[2]
...
export default (connect(mapStateToProps, mapDispatchToProps)(SpeedLimitExceeded))
