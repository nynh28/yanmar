...
import {
  ...
  getGeoServerUrl,
  YM_BASE_URL,
  ENDPOINT_BASE_URL_YNM_VNTEAM,
} from "../../Config/app-config";
import { getIconStatusPathTractor } from "../../Functions/StatusColor";
...
const Map = () => {
  ...
  const filterMapTractorStr = useSelector((state) => state.realtimeNew.filterMapTractorStr);
  ...
  const initial_lang = language;
  const prevProps = useRef(initial_lang);
  ...
  useEffect(() => {
    // if(filterMapTractorStr){
    //   console.log(filterMapTractorStr)
    //   // return () => {
    //     // componentWillUnmount
    //     // cancleFetchApi();
    //     setTimeout(_ => {
  ...
  const loadRealtimeInitial = async () => {
    ...
    if (!eventMapActive) {
      ...
      try {
        ...
        console.log(filterMapTractorStr)
        // var response = await fetch(ENDPOINT_REALTIME_V2 + "fleet/V2/Realtime?user_id=" + dataLogin.user_id + "&gzip_status=True", {
        var response = await fetch(
          `${ENDPOINT_BASE_URL_YNM_VNTEAM}fleet/dlt/excavator/realtime1?user_id=${dataLogin.user_id}${filterMapTractorStr}`,
          {
            method: "GET",
            headers: {
        ...
        if (get(document, "location.hash") !== "#/Tractor/realtime") return;
        ...
        if (!isEmpty(data)) {
          ...
          let listId = dataVehicles.map(i => i.vid)
          let displayVehicle1 = JSON.parse(JSON.stringify(listId))
  ...
  const loadRealtimeInitialDash = async () => {
    ...
    if (!eventMapActive) {
      ...
      try {
        ...
        // var response = await fetch(ENDPOINT_REALTIME_V2 + "fleet/V2/Realtime?user_id=" + dataLogin.user_id + "&gzip_status=True", {
        var response = await fetch(
          `${YM_BASE_URL}fleet/dlt/excavator/realtime2?user_id=${dataLogin.user_id}${filterMapTractorStr}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
  ...
  const loadRealtimeLast = async () => {
    if (!eventMapActive && !requestWorking) {
      ...
      try {
        ...
        let signal = controller.signal;
        controllerSignal.push(controller);
        // var response = await fetch(ENDPOINT_REALTIME + "fleet/GunicornRealtime?user_id=" + dataLogin.user_id + "&last_call=" + _timeLast + "&gzip_status=True", {
        // var response = await fetch(ENDPOINT_REALTIME_V2 + "fleet/V2/Realtime?user_id=" + dataLogin.user_id + "&last_call=" + _timeLast + "&gzip_status=True", {
        axios
          .get(
            `${ENDPOINT_BASE_URL_YNM_VNTEAM}fleet/dlt/excavator/realtime1?user_id=${dataLogin.user_id}&last_call=${_timeLast}${filterMapTractorStr}`
          )
          .then((res) => {
            var data = res.data;
            _timeLast = new Date().toISOString().split(".")[0];
            ...
            if (!isEmpty(data)) {
              // loadRealtimeExtend(false, data, dataLogin.user_id)
              let dataVehicles = get(data, "sidebar", []);
              // let { dash_count_bar, dash_behaviour_bar, maintenance_status } =
              //   data;
            ...
            if (window.location.hash === "#/Tractor/realtime")
              countdownUpdate();
          });

        // var response = await fetch(
        //   `https://192.168.3.45:5001/prod/fleet/dlt/excavator/realtime?user_id=${dataLogin.user_id}&last_call=${_timeLast}`,
        //   {
        //     method: "GET",
        //     headers: {
        //       Accept: "application/json",
        ...
        if (window.location.hash === "#/Tractor/realtime") countdownUpdate();
  ...
  const loadRealtimeLast2 = async () => {
    ...
    if (!eventMapActive && !requestWorking) {
      ...
      try {
        ...
        axios
          .get(
            `${ENDPOINT_BASE_URL_YNM_VNTEAM}fleet/dlt/excavator/realtime2?user_id=${dataLogin.user_id}&last_call=${_timeLast}${filterMapTractorStr}`
          )
          .then((res) => {
            var data = res.data;
            _timeLast = new Date().toISOString().split(".")[0];
  ...
  return (
    <div style={{ marginTop: 6 }}>
      ...
      <div className="realtime-tractor" style={{ height: "calc(100vh - 71px)", width: "100%" }}>
        <GoogleMapReact
          bootstrapURLKeys={{
            key:
              GOOGEL_MAP_API_KEY
        ...
        >
          {map !== null && (
            <>
              ...
              {/* MODAL VIDEO */}
              {/* {dataLogin.user_id === 36 && <MapControl position={5} map={map} id={'layout-modal-video'} width='auto'>
                <ModalVideo />
              </MapControl>} */}
