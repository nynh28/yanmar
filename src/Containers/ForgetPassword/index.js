...
class ForgetPassword extends Component {
  ...
  componentWillMount() {
    ...
    if (timeStart && ((new Date()).getTime() / 1000 | 0) - timeStart < 30) {
      ...
      this.cooldownButton()
    } else if (timeStart && ((new Date()).getTime() / 1000 | 0) - timeStart > 30) {
  ...
  render() {
    let { emailOrMobile, newPassword, conNewPassword, confirmCode, passNotEqual, respone } = this.state
      ...
      <Suspense fallback={null}>
        <div
          style={{
            ...
            backgroundColor: '#DCDCDC', backgroundImage: `url(${require('./yanmar-bg.png')})`, backgroundSize: 'cover'
          }}>

          <Row style={{ position: 'absolute', height: '100%', width: '100%' }}>
            <Col lg='12' style={{ display: 'flex', height: '100%', justifyContent: 'left', alignItems: 'center' }}>
              <div className="box-forgotpassword"  >
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 10 }}>
                  <img alt="image" src={Images.logoYanmar} style={{ position: 'relative', width: '230px', marginBottom: '15px' }} />
                </div>

                {this.state.isActive ?
                  // ------------------ Email or Mobile ------------------
                  <div style={{ display: 'flex', flex: 1, height: '100%', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                    <span style={{ fontSize: 22, marginBottom: 15 }}>{t('forgot_password')}</span>
                    <div className="form-group field field-string" style={{ padding: '0 10px', flex: 1 }}>
                      <label className="control-label" style={{ fontWeight: 500 }}>{t("email_or_mobile")}</label>
                      <Input
                        size="large"
                        placeholder={rTT(t("email_or_mobile"))}
                        onChange={(e) => this.setState({ emailOrMobile: e.target.value.trim() })}
                        onKeyDown={(e) => { if (e.key === 'Enter') this.sendEmailOrMobile() }}
                      />
                    </div>
                    {/* --------------------- */}
                    <span style={{ color: 'red', marginTop: 10 }}>{errorSubcodeForget && getErrorMessage(parseInt(errorSubcodeForget))}</span>
                    <div style={{ display: 'flex', marginTop: 10 }}>
                      <Button
                        className="btn-forgot-submit"
                        text={countCoolDown > 0 ? countCoolDown : 'submit'}
                        backgroundColor="rgb(219 31 55)"
                        borderColor="rgb(219 31 55)"
                        width={300}
                  ...
                  <div style={{ display: 'flex', flex: 1, height: '100%', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <div className="form-group field field-string" style={{ padding: '0 10px', flex: 1 }}>
                      <label className="control-label" style={{ fontWeight: 500 }}>{t("forgot_password_1")}</label>
                      <Input
                        size="large"
                        placeholder={rTT(t("forgot_password_1"))}
                        value={confirmCode}
                        onChange={(e) => this.setState({ confirmCode: e.target.value.trim() })}
                      />
                    </div>

                    <div className="form-group field field-string" style={{ padding: '0 10px', flex: 1 }}>
                      <label className="control-label" style={{ fontWeight: 500 }}>{t("forgot_password_2")}</label>
                      <Input.Password
                        size="large"
                    ...
                    <div className="form-group field field-string" style={{ padding: '0 10px', flex: 1 }}>
                      <label className="control-label" style={{ fontWeight: 500 }}>{t("forgot_password_3")}</label>
                      <Input.Password
                        size="large"
                    ...
                    <span style={{ color: 'red' }}>
                      {t(passNotEqual)}
                      {errorSubcodeConfirm && getErrorMessage(parseInt(errorSubcodeConfirm))}
                    </span>
                    <div style={{ display: 'flex', marginTop: 10 }}>
                      <Button
                        className="btn-resend-password"
                        text={countCoolDown > 0 ? countCoolDown : 'resend_password'}
                        backgroundColor="#FFF"
                        borderColor="#a1a1a1"
                        textColor="#000"
                        width={180}
                        ...
                        onClick={() => this.props.forgotPassword(this.state.emailOrMobile)}
                      />
