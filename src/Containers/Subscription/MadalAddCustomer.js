...
import { FormInput, FormLabel, FormSelectSearch, FormRadio, FormTextArea, FormDatepicker } from '../../Components/FormControls'
...
class MadalAddCustomer extends Component {
  ...
  async createCustomer() {
    ...
    let { prefix, firstname, lastname, isIndividual, businessType,
      taxId, phoneNo, email, lineId, officialAddress } = this.state
    try {
      ...
      var response = await fetch(`${ENDPOINT_BASE_URL}ServiceContract/Yanmar/Customer`, {
        method: 'POST',
        headers: {
          "Accept": 'application/json',
          'Content-Type': 'application/json',
  ...
  onCheckRequired() {
    ...
    if (messageList.length > 0)
      this.setAlert(true, 4, "", messageList)
    else
  ...
  render() {
    ...
    const { businessTypeList, isIndividual, taxId, prefix, firstname,
      lastname, phoneNo, businessType, lineId, email, officialAddress,
      alertSetting, loading
    } = this.state
            ...
            <Row>
              <Col lg={6} md={12}>
                <FormSelectSearch
                  ...
                  onChange={(value) => this.setState({ businessType: value !== undefined ? value : "" })}
                />
              </Col>
              <Col lg={6} md={12}>
                <FormInput
