...
class NotifyDriverData extends Component {
  constructor(props) {
    super(props);
    console.log(props);
    this.state = {
  ...
  openModal() {
    this.setState((state) => ({
        showFormPopupNotifyDriver: !state.showFormPopupNotifyDriver,
        // groupId: [],
        // dupData: false 
      }), () => {
  ...
  onRowInserting() {
    ...
    for(let i=0; i <= userId.length-1; i++){
      let obj = schema.list.userDriver.find(x => x.key === parseInt(userId[i]));
      ...
      <Suspense fallback={null}>
        <div style={{ display: "flex", flexDirection: "row" }}>
          <div style={{ display: "flex", flexDirection: "column", flex: 2 }}>
            ...
            <Row>
              <Col lg={6} md={12}>
                <FormRadio
                  schema={schema}
                  value={notifyDriver}
                  label={schema.label.notifyDriver}
                  fieldForm={"notifyDriver"}
                  flex={2}
                  onClick={(notifyDriver, fieldForm) => {
                    this.onCheckedButton(notifyDriver, fieldForm);
                  }}
                />
              </Col>
            </Row>

            {this.state.notifyDriver == true && (
              <div>
                <Row>
                  <Col lg={6} md={12}>
                    <FormRadio
                      schema={schema}
                      value={notifyLevelDriver}
                      label={schema.label.notifyLevel}
                      fieldForm={"notifyLevelDriver"}
                      flex={2}
                      name={["Normal", "Important"]}
                      onClick={(notifyLevelDriver, fieldForm) => {
                        this.onCheckedButton(notifyLevelDriver, fieldForm);
                      }}
                    />
                  </Col>
                  <Col lg={6} md={12}>
                    <FormInput
                      ...
                      fieldForm={"subjectDriver"}
                      flex={2}
                      onChange={this.onChange("subjectDriver")}
                    />
                  </Col>
                </Row>
                ...
                <Row>
                  <Col lg={6} md={12}>
                    <FormTextArea
                      ...
                      fieldForm={"messageDriver"}
                      flex={2}
                      onChange={this.onChange("messageDriver")}
                    />
                  </Col>
                  <Col lg={6} md={12}>
                    <FormRadio
                      schema={schema}
                      value={specifyUserDriver}
                      label={schema.label.specifyUser}
                      fieldForm={"specifyUserDriver"}
                      flex={2}
                      onClick={(specifyUserDriver, fieldForm) => {
                        this.onCheckedButton(specifyUserDriver, fieldForm);
                      }}
                    />
                  </Col>
                </Row>
            ...
            {/* <Row>
              ...
              <Col lg={6} md={12}>
                <FormRadio
                  ...
                  fieldForm={"subjectDriver"}
                  flex={2}
                  onClick={(subjectDriver, fieldForm) => {
                    this.onCheckedButton(subjectDriver, fieldForm);
                  }}
                />
              </Col>
            </Row>
            ...
            <Row>
              <Col lg={6} md={12}>
                <FormRadio
                  ...
                  fieldForm={"messageDriver"}
                  flex={2}
                  onClick={(messageDriver, fieldForm) => {
                    this.onCheckedButton(messageDriver, fieldForm);
                  }}
                />
              </Col>
              <Col lg={6} md={12}>
            ...
            <Modal
              title={t("add_user")}
              visible={showFormPopupNotifyDriver}
              okText={t("add")}
              cancelText={t("cancel")}
            ...
            >
...

              {
                ...
                <FormSelectSearch
                  ...
                  schema={schema}
                  value={userId}
                  label={schema.label.userDriver}
                  list={schema.list.userDriver}
                  // list={[]}
                  fieldForm={"userId"}
                  placeholder={"placeholder"}
                  flex={1}
...
export default connect(mapStateToProps, mapDispatchToProps)(NotifyDriverData);
