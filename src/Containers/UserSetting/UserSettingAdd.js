...
class UserSettingAdd extends Component {
  constructor(props) {
    super(props)
    this.state = {
      username: '',
  ...
  componentDidMount() {
    if (this.props.groupType == 'Hino') {
      this.setState({ i: 1, groupType: this.props.groupType.toLowerCase() })
    }
    else if (this.props.groupType == 'Dealer') {
      this.setState({ i: 0, groupType: this.props.groupType.toLowerCase() })
    }
    else {
      this.setState({ groupType: this.props.groupType.toLowerCase() + 's' })
    }
  ...
  onClickSave = () => {
    ...
    if (this.state.username && this.state.fullname && this.state.aliasName && this.state.email && this.state.temporaryPassword && this.state.businessPartnerId && this.state.suffixRoleName) {
      // console.log('saving...')
      this.props.addUser({ type: this.state.groupType, id: this.props.businessPartnerId, userRequest: { username: this.state.username, fullname: this.state.fullname, aliasName: this.state.aliasName, email: this.state.email, temporaryPassword: this.state.temporaryPassword, businessPartnerId: this.state.businessPartnerId, suffixRoleName: this.state.suffixRoleName } })
    }
    else {
...
export default connect(mapStateToProps, mapDispatchToProps)(UserSettingAdd)
