...
class Map extends Component {
  ...
  componentDidUpdate(prevProps) {
    let { searchData, select, vid, initialVehiclesDataSI, zoom } = this.props;
    ...
    if (
      prevProps.initialVehiclesDataSI !== initialVehiclesDataSI &&
      prevProps.select === select
    ) {
      // console.log("xxxxx :", initialVehiclesDataSI.length)

      // let ele = document.getElementById('total-vehicle')
      // if (ele) ele.textContent = numberWithComma(initialVehiclesDataSI.length)
  ...
  mapVid = (vid) => {
    const e = this.props;
    // console.log("vid :", vid)
    const data = e.initialVehiclesDataSI.find((e) => e.vid === vid);
  ...
  toggleDashboard() {
    ...
    this.props.onDashcboard &&
      this.props.onDashcboard(this.state.dashboardEnabled);
  }
  ...
  render() {
    let { centerDefualt, isShow, size, yAxis, zoomDefault, geofenceByTypes } =
      this.state;
    let { searchData, vid, geofencesEnabled, initialVehiclesDataSI, zoom } =
      this.props;
    ...
    return (
      <LoadScriptNext
        id="script-loader"
        googleMapsApiKey={
          "AIzaSyDNwWt4rwJJ953rjpfYPJwoYYfq-JYYZ1A&libraries=geometry"
        }
      >
        <GoogleMap
          onLoad={(map) => {
              ...
              <>
                <MapControlsCustom
                  ...
                  onObjectChange={(value) =>
                    console.log("onObjectChange", value)
                  }
                  onImeasureChange={(value) =>
                    console.log("onImeasureChange", value)
                  }
                  onDashboardChange={(value) =>
                    this.props.onDashboardChange(value)
                  }
                ...
                <MarkerCluster
                  map={this.map}
                  // initialVehiclesDataSI={initialVehiclesDataSI}
                  clusterEnabled={this.state.clusterEnabled}
                  size={size}
                  yAxis={yAxis}
                />
                <MarkerActive map={this.map} size={size} yAxis={yAxis} />
              </>
            )}
          </>
        </GoogleMap>
...
const mapDispatchToProps = (dispatch) => ({
  setList: (initialVehiclesDataSI) =>
    dispatch(ControlRoomDealerActions.setList(initialVehiclesDataSI)),
  setZoom: (zoom) => dispatch(ControlRoomDealerActions.setZoom(zoom)),
  getGeofenceByTypes: (GeofenceTypeIds) =>
    dispatch(RealtimeActions.getGeofenceByTypes(GeofenceTypeIds)),
});
