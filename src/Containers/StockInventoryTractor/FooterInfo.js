...
class FooterInfo extends Component {
    ...
    if (nextProps.information !== information) {
      if (!isEmpty(nextProps.information)) {
        if (isEmpty(information)) {
          this.getInformationExtend(nextProps.information.vid);
        } else if (
          nextProps.information.vid !== information.vid ||
          isLoadInfo
        ) {
          this.getInformationExtend(nextProps.information.vid);
        }
  ...
  async getInformationExtend(vid) {
    try {
      ...
      if (!isEmpty(data)) {
        ...
        console.log("extend", infoUpdate.infoExtend);
  ...
  render() {
    ...
    let { loadingVideo, informationData } = this.state;
    let place = "";
    if (language === "en") {
    ...
    console.log(informationData.vin_no);
    ...
    let info = {
      ...
      vehicle: get(informationCR, "vin_no", null),
      gpsdate: momentDate(
        get(informationCR, "gpsdate")
          ? get(informationCR, "gpsdate", null)
          : get(informationCR, "gpsdate", null)
      ),
      workHours: getStatusVehicle(get(informationCR, "my_drivers_21")),
      model_code: get(informationCR, "model_code", "-"),
      rpm: get(informationCR, "rpm", 0),
      sattelliteLevel: get(informationCR, "sattellite_level", 0),
      sattellitePer: Number(get(informationCR, "sattellitePer", 0)),
      gsmLevel: get(informationCR, "gsm_level", "-"),
      gsmPer: Number(get(informationCR, "gsm_per", "-")),
      deviceStatus: get(informationCR, "device_batt", 0) <= 0 ? false : true,
      location:
        get(informationCR, "lat", "-") + ", " + get(informationCR, "lng", "-"),
      place: place === "" ? "-" : place,
      vehicleBattLevel: this.colorAndNameBatt(
        get(informationCR, "vehicleBattLevel")
      ),
      vehicleBatt: get(informationCR, "vehicle_batt", "-"),
      deviceBattLevel: this.colorAndNameBatt(
        get(informationCR, "device_batt_level")
      ),
      deviceBatt: get(informationCR, "device_batt", "-"),
      fuel: get(informationCR, "fuel_per", "-"),
      pto: "",
      dtc_engine: this.onOff(get(informationCR, "dtc_engine")),
      sleep_mode: get(informationCR, "sleep_mode", "0"),
      vin_no: get(informationCR, "vin_no", ""),
      icon: getIconStatusPathTractor(
        parseInt(get(informationCR, "status", 99))
      ),
      statusName: getStatusName(get(informationCR, "status", 99)),
      dealer_name: get(informationCR, "dealer_name", ""),
      delivery_date: get(informationCR, "delivery_date", ""),
      finance_contract_number: get(
        informationCR,
        "finance_contract_number",
        ""
      ),
      model_name: get(informationCR, "model_name", ""),
      next_service: get(informationCR, "next_service", ""),
      phone: get(informationCR, "phone", ""),
      status: get(informationCR, "status", ""),
      customer_name: get(informationCR, "customer_name", ""),
      engine_hour: get(informationCR, "engine_hour", ""),
      coolant_temp: get(informationCR, "coolant_temp", ""),
    };
    ...
    return (
      console.log("info", informationCR),
      !isEmpty(info) && (
        // return ((true) &&
        <Row style={{ margin: "0px -10px -15px 0px" }}>
          <div div style={{ width: 80 }}>
            <div
              ...
              onClick={() => {
                this.props.setHideFooter({ hideFooter: !hideFooter });
                // this._onCollapseComponent()
              }}
            >
              <center>
