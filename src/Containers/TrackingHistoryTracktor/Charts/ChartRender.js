...
const ChartRenders = (arg) => {
  ...
  let hideTitle = false, config = arg.config, speed_limit = 0, temp_limit = 0, isDynamic = false, rpm_zone = arg.rpm_zone
  ...
  >
    ...
    {
      isDynamic ? arg.option_temperature.map((item) => {
        return (<Series
        ...
        : config.Series.map((item, i) => {
          return (<Series
            ...
            name={t(item.name)}
            color={get(arg.chartColour, "chart_colors." + item.color, item.color)}
            border={{
              color: "#595959",
              width: 1,
              visible: true
    ...
    >
      ...
      {
        arg.chartTypeId === 2 && get(arg, 'rpm_zone') && get(arg.chartColour, 'chart_colors') && [
          (rpm_zone.red_zone[0] !== 0 && rpm_zone.red_zone[1] !== 0) &&
          <Strip startValue={rpm_zone.red_zone[0]} endValue={rpm_zone.red_zone[1]} color={arg.chartColour.chart_colors.rpm_zone.red_zone} />,
          (rpm_zone.green_zone[0] !== 0 && rpm_zone.green_zone[1] !== 0) &&
          <Strip startValue={rpm_zone.green_zone[0]} endValue={rpm_zone.green_zone[1]} color={arg.chartColour.chart_colors.rpm_zone.green_zone} />
        ]
      }

      <Grid visible={config.ValueAxis.grid.visible || false} />
      {
        get(config.ValueAxis, 'label.customizeText', false) && <Label customizeText={arg.customizeTextSensor} />
      }

      {
        get(config.ValueAxis, 'constantLine', false) && <ConstantLine
          value={arg.chartTypeId === 1 ? parseInt(speed_limit) : arg.chartTypeId === 7 ? parseInt(temp_limit) : 0}
          // value={50}
          width={2}
          // color={get(chart_colors, config.ValueAxis.constantLine.color)}
          color={get(arg.chartColour, "chart_colors." + config.ValueAxis.constantLine.color)}
          dashStyle="Dash"
        >
          <Label visible={false} />
        </ConstantLine>
    ...
    <Tooltip
      ...
      customizeTooltip={(e) => {
        let data = e.point.data, element = ""

        for (let idx in arg.option_temperature) {
          element += '<i class="fa fa-square" style="color: ' + arg.option_temperature[idx].color +
            '; font-size: 10px;"></i><span style="margin-left: 5px; font-size: 12px; padding-right: 20px;">' + arg.option_temperature[idx].name + ' : ' + data['temp_' + arg.option_temperature[idx].seq] + '</span><br>'
        }
...
class ChartRender extends Component {
    ...
    try {
      ...
      if (index < points.length) {
        if (chartVisibled.includes(1)) {
          ...
          $("#chart_1 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_1 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(2)) {
          ...
          $("#chart_2 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_2 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(3)) {
          ...
          $("#chart_3 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_3 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(4)) {
          ...
          $("#chart_4 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_4 .dxc-crosshair-cursor g path").attr("d", "M 31 0 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(5)) {
          ...
          $("#chart_5 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_5 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(6)) {
          ...
          $("#chart_6 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_6 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(7)) {
          ...
          $("#chart_7 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_7 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
        ...
        if (chartVisibled.includes(8)) {
          ...
          $("#chart_8 .dxc-crosshair-cursor g").attr("transform", "translate(" + (points[index].x - 31) + ", 0)")
          $("#chart_8 .dxc-crosshair-cursor g path").attr("d", "M 31 8 L 31 " + crosshairHeight)
  ...
  render() {
    let { chartConfig, chartTypeId, dataAllPoint, config, defaultVisualRange, rpm_zone, option_temperature } = this.props
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setAfterZoomChart: (value) => dispatch(TrackingHistoryActions.setAfterZoomChart(value)),
})
