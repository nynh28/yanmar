...
class TrackingHistory extends Component {
  ...
  componentWillMount() {
    ...
    if ([31, 32].includes(dataLogin.user_level_id)) this.isDealer = true;
    else if ([41, 42].includes(dataLogin.user_level_id)) this.isCustomer = true;
    else if ([21, 22].includes(dataLogin.user_level_id)) this.isHino = true;
  ...
  async getalldata() {
    try {
      var response = await fetch(YM_BASE_URL + "fleet/getV", {
        ...
        body: JSON.stringify({
          user_id: this.props.dataLogin.user_id,
        }),
      });
  ...
  async load_manage_customer() {
    let userId = this.props.dataLogin.user_id;
    var response = await fetch(YM_BASE_URL + "fleet/get_customer_by_manage", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  async getFleet(cust_id) {
    var object = {
      userId: this.props.dataLogin.user_id,
    };
    try {
      var response = await fetch(YM_BASE_URL + "fleet/getfleet", {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  async loadListMember(cust_id, fleetId) {
    ...
    let userId = this.props.dataLogin.user_id;
    let url =
      YM_BASE_URL + "fleet/dropdown?user_id=" + userId + "&options=VEHICLE";
  ...
  async searchData() {
    ...
    this.props.setValue("isLoadingTrips", false);
    // this.props.setValue("indexTripSelected", [])
  }

  getColorIndex(seq) {
    let colorTemp = this.props.config.chart_colors.option_temperature,
      color = "#000";
  ...
  addFieldData(data, language) {
    ...
    for (let index in data) {
      ...
      if (trip.length == 22) trip[21] = this.setEventName(trip[2], language);
  ...
  render() {
    ...
    // console.log(">> RENDER TRACKING HISTORY <<")
                ...
                <Col lg={6} sm={5}>
                  <Select
                    ...
                    onChange={(value, id) => {
                      ...
                      console.log("currentValue", chassisNo);
                    }}
                    onSearch={(input) => {
                      this.setState({
