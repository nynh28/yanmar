...
import {
  momentDate,
  calculateToDuration,
  calculateToTimestamp,
} from "../../../Functions/DateMoment";
import { getSum, getAvg } from "../Functions";
import { exportExcel } from "../../../Functions/ExportExcel";
import { get } from "lodash";
import moment from "moment";
import { getEventIconTracktor, getEventName } from "../Functions";
...
const ColumnHint = (arg) => {
  ...
  return (
    <td
      title={
        arg.keyName !== undefined
          ? t(arg.keyName)
          : t(getEventName(arg.eventId))
      }
    >
      {arg.content}
    </td>
...
class OverlayPanel extends Component {
        ...
        <tr style={{ cursor: "pointer" }}>
          ...
          <td>
            <span style={{ fontSize: 12 }}>
              {moment(trip[0]).format("DD/MM/YYYY HH:mm:ss")}
            </span>
          </td>
          {/* <td>Loylum Bunnag</td> */}
          <td> </td>
      ...
      tbRow.push(
        <tr style={{ cursor: "pointer" }} onClick={() => this.selectTrip(trip)}>
          <ColumnHint
            eventId={trip[2]}
            content={
              "" + getEventIconTracktor(trip[2]) === "unknow" ? (
                <span className="action-icon startstop-action">{trip[2]}</span>
              ) : (
                <img
                  src={getEventIconTracktor(trip[2])}
                  alt="image"
          ...
          <td>
            ...
            <span style={{ fontSize: 12 }}>
              {event_id_list.includes(get(trip, "[2]", 0))
                ? trip[2] === 1001
                  ? t("history_62") + " (" + get(trip, "[3]", "-") + ")"
                  : trip[2] === 1002
                  ? t("history_63") + " (" + get(trip, "[3]", "-") + ")"
                  : get(trip, "[3]", "-")
                : calculateToDuration(get(trip, "[8]", 0))}
            </span>
          </td>
        </tr>
      );
    ...
    if (typeTrip == "ENDONLIY") {
            ...
            <ColumnColspanHint
              ...
              content={
                <div>
                  ...
                  <span style={{ fontSize: 12, paddingLeft: 10 }}>
                    {moment(trip[0]).format("DD/MM/YYYY HH:mm:ss")} -{" "}
                    {moment(tripEnd[0]).format("DD/MM/YYYY HH:mm:ss")}
                  </span>
                </div>
              }
            />
      ...
      tbRow.push(
        <tr
          style={{
            cursor: "pointer",
            backgroundColor: this.props.subRootTrip.includes(trip[2])
              ? "rgb(249 249 249)"
              : "",
          }}
          onClick={() => this.selectTrip(trip)}
          ...
          <ColumnHint
            ...
            content={
              "" + getEventIconTracktor(trip[2]) === "unknow" ? (
                ...
                <div
                  style={{
                    paddingLeft: this.props.subRootTrip.includes(trip[2])
                      ? 20
                      : 0,
                  }}
                >
  ...
  fitBoundsEvent(path) {
    ...
    if (map) {
      ...
      const removeDuplicatesFromArray = (arr) =>
        [...new Set(arr.map((el) => JSON.stringify(el)))].map((e) =>
          JSON.parse(e)
        );
  ...
  createGroupCamera(data, key) {
    return (
      <Panel
        header={moment(data.take_photo_time, "YYYY-MM-DD HH:mm:ss").format(
          "DD/MM/YYYY HH:mm:ss"
        )}
        key={key}
          ...
          {data.channelList.map((item, i) => {
            return (
              <div
                ...
                onClick={() =>
                  this.props.setValue("indexImage", {
  ...
  render() {
    ...
    return [
      <span style={{ height: "100%", paddingTop: 0 /*165*/ }}>
        ...
        >
          ...
          <center>
            <i
              className={
                "fa " + (hidePanelBox ? "fa-chevron-left" : "fa-chevron-right")
              }
            ></i>
          </center>
        </div>
      ...
      <div
        className="box-panel-trip"
        style={{ display: hidePanelBox ? "none" : "inline-block" }}
      >
        ...
        <Tabbed
          ...
          tabName={[
            <span>
              <i className="fa fa-history" title="History" />
            </span>,
          ]}
          tabPane={[
            // VehicleList
            <div>
              ...
              />
              <div
                className="box-panel-trip-list list-history-scrollbar scrollbar-custom"
                id="style-3"
              >
                <table
                  className="table table-hover table-bordered tb-trip-list header-fixed"
                ...
                >
                  ...
                  <tbody>
                    ...
                    {tripList.length == 1 &&
                      this.createRow(tripList[0], "ENDONLIY")}
                  </tbody>
                </table>
              </div>
            </div>,
  ...
  setupExport() {
    ...
    if (data.length > 0) {
      ...
      let end_date = moment(
        get(data, "[" + (data.length - 1) + "][0]"),
        "YYYY-MM-DD HH:mm:ss"
      );

      text.push(
        start_date.format("DD/MM/YYYY HH:mm:ss"),
        " - ",
        end_date.format("DD/MM/YYYY HH:mm:ss")
      );
      file_name +=
        start_date.format("DD-MM-YYYY HH_mm_ss") +
        " to " +
        end_date.format("DD-MM-YYYY HH_mm_ss");
    } else {
      text.push("-");
      file_name += "-";
    }
    ...
    if (option_temperature.length > 0) {
      ...
      custom_header.list.push({
        text: [t("history_44")],
        index_cell: 25,
        merge_cell: [25, 24 + option_temperature.length],
        font: { bold: true },
  ...
  changeFormatJSON(dataJson, license_plate_no) {
    ...
    newData = dataJson.map((item, i) => {
      ...
      if (option_temperature.length > 0)
        option_temperature.forEach((value, idx) => {
  ...
  statusName(speed) {
    ...
    if (speed_limit > 0 && speed > speed_limit)
      name = language == "en" ? "Over Speed" : "ความเร็วเกิน 60 กม./ชม.";
    else if (speed <= 2)
      name = language == "en" ? "Idling" : "จอดไม่ดับเครื่อง";
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(OverlayPanel)
);
