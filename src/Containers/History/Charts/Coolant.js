...
class Coolant extends Component {
  ...
  componentDidUpdate(prevProps, nextState) {
    ...
    if (prevProps.markTourInfo.isMarkPlay != false
      ...
      if (markTourInfo.isMarkPlay) {
        dataTour.push({
          coolant: dataAllPoint[markTourInfo.index].coolant,
  ...
  customizeTooltip(arg) {
    ...
    this.props.setPointValue({ value: arg.valueText, unit: '°C', date: gpsdate })
  }
  ...
  componentWillMount() {
    ...
    if (plotData.length === 0) {
      ...
      for (let idx in dataAllPoint) {
        plotData.push(
          {
            coolant: dataAllPoint[idx].coolant,
...
export default connect(mapStateToProps, mapDispatchToProps)(Coolant)
