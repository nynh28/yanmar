...
class Geofences extends Component {
  ...
  componentDidMount() {
    this.props.geofenceByTypes.length > 0 &&
      this.setObjectData(this.props.geofenceByTypes);
  }
  ...
  shouldComponentUpdate(nextProps) {
    ...
    if (language !== nextProps.language)
      this.setTitleInfoWindow(nextProps.language);

    if (geofenceDetail !== nextProps.geofenceDetail) {
      $("#lb_geofenceType").text(
        get(nextProps.geofenceDetail, "geofenceTypeNav.value")
      );
      $("#lb_geofenceName").text(get(nextProps.geofenceDetail, "name"));
      $("#lb_geofenceDescription").text(
        get(nextProps.geofenceDetail, "description")
      );
      $("#lb_subdistrict").text(
        get(nextProps.eofenceDetail, "locationSimpleInfo.subdistrict")
      );
      $("#lb_district").text(
        get(nextProps.geofenceDetail, "locationSimpleInfo.district")
      );
      $("#lb_province").text(
        get(nextProps.geofenceDetail, "locationSimpleInfo.province")
      );
      $("#lb_hazard").text(
        get(nextProps.geofenceDetail, "isHazard") ? "Yes" : "No"
      );
      $("#lb_share").text(
        get(nextProps.geofenceDetail, "isShare") ? "Yes" : "No"
      );
      $("#lb_geometryType").text(
        get(nextProps.geofenceDetail, "geomTypeNav.value")
      );
    }
  ...
  clearAllObject() {
    ...
    for (let index in this.markerPolylines)
      this.markerPolylines[index].setMap(null);

    // Clear Polygon
    for (let index in this.polygons) this.polygons[index].setMap(null);
    for (let index in this.markerPolygons)
      this.markerPolygons[index].setMap(null);

    // Clear Polygon
    for (let index in this.circles) this.circles[index].setMap(null);
    for (let index in this.markerCircles)
      this.markerCircles[index].setMap(null);
  ...
  async getGeofenceDetail(id) {
    ...
    var response = await fetch(
      ENDPOINT_BASE_URL + "Geofence/Geofence/" + id + "?lang=" + language,
      {
        method: "GET",
        headers: {
    ...
    this.props.setStatesGeofencesRedux({ geofenceDetail: data });
  }
...
const mapStateToProps = (state) => ({
  ...
  geofenceDetail: state.geofences.geofenceDetail,
});
