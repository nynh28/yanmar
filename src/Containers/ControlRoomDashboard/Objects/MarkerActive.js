...
import { statusCarOld } from "./StatusVehicle";
...
class MarkerActive extends Component {
  ...
  render() {
    ...
    let status = statusCarOld(informationCR);

    let obj = iconActive.find(
      (x) =>
        x.classType === parseInt(get(informationCR, "info.class_type", 0)) &&
        x.status === status
    );
    ...
    return (
      !isEmpty(informationCR) && (
        <Marker
          ...
          icon={
            ...
            ${encodeURIComponent(
              icon
                .replace("{{width}}", size)
                .replace("{{height}}", size)
                .replace("{{transform}}", `rotate(${informationCR.gps.course})`)
            )}`,
          ...
          position={{ lat: informationCR.gps.lat, lng: informationCR.gps.lng }}
          title={informationCR.info.licenseplate}
        >
          {/* <InfoBox
        // onLoad={onLoad}
        options={{
