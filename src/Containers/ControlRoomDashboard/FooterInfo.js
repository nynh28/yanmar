...
import { ENDPOINT_BASE_URL_YNM4, YM_BASE_URL } from "../../Config/app-config";
...
class FooterInfo extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    let { listVideo, isShowModalVideo, informationCR, isLoadInfo } = this.props;

    if (nextProps.informationCR !== informationCR) {
      if (!isEmpty(nextProps.informationCR)) {
        if (isEmpty(informationCR)) {
          this.getInformationExtend(nextProps.informationCR.vid);
        } else if (
          nextProps.informationCR.vid !== informationCR.vid ||
          isLoadInfo
        ) {
          this.getInformationExtend(nextProps.informationCR.vid);
        }
  ...
  async getInformationExtend(vid) {
    try {
      ...
      if (!isEmpty(data)) {
        let infoUpdate = { ...this.props.informationCR };
        ...
        infoUpdate.infoExtend = {
          ...
          engine_hour: parseInt(parseInt(data?.engine_hour) / 60),
        ...
        this.props.setInformation(infoUpdate);
  ...
  getLocation(place, node) {
    let name = get(this.props.informationCR, node, "");
    if (name !== "") place += name + " ";
    return place;
  }
  ...
  changeChanel(isNext) {
    let { listVideo, informationCR } = this.props;
    let channel = get(listVideo, "[0].channel");
    let lst = get(informationCR, "['option.mdvr'].channel", []);
  ...
  render() {
    ...
    let information = informationCR;
    let place = "";
    if (language === "en") {
      place = this.getLocation(place, "location.admin_level3_name_en");
      place = this.getLocation(place, "location.admin_level2_name_en");
      place = this.getLocation(place, "location.admin_level1_name_en");
    } else {
      place = this.getLocation(place, "location.admin_level3_name");
      place = this.getLocation(place, "location.admin_level2_name");
      place = this.getLocation(place, "location.admin_level1_name");
    }
    ...
    let info = {
      ...
      vehicle: get(information, "info.vehicle_name", null)
        ...
        : get(information, "vin_no", null),
      gpsdate: momentDate(
        get(information, "gpsdate")
          ? get(information, "gpsdate", null)
          : get(information, "gpsdate", null)
      ),
      ...
      dtc_engine: this.onOff(get(information, "dtc_engine")),
      ...
      model_code: get(information, "model_name", "-"),
      rpm: get(information, "rpm", 0),
      nameDriver:
      ...
      sattelliteLevel: get(information, "sattellite_level", 0),
      sattellitePer: Number(get(information, "sattellite_per", 0)),
      gsmLevel: get(information, "gsm_level", "-"),
      gsmPer: Number(get(information, "gsm_per", 0)),
      deviceStatus: get(information, "device_batt", 0) <= 0 ? false : true,
      location:
        get(information, "lat", "-") + ", " + get(information, "lng", "-"),
      place: place === "" ? "-" : place,
      cooltemp: get(information, "sensor.canbus.cooltemp", "-"),
      vehicleBattLevel: this.colorAndNameBatt(
        get(information, "vehicle_batt_level")
      ),
      vehicleBatt: get(information, "vehicle_batt", "-"),
      deviceBattLevel: this.colorAndNameBatt(
        get(information, "device_batt_level")
      ),
      deviceBatt: get(information, "device_batt", "-"),
      accPedal: get(information, "acc_pedal", "-"),
      fuel: get(information, "fuel_per", "-"),
      ...
      sleep_mode: get(information, "sleep_mode", "0"),
      vin_no: get(information, "vin_no", ""),
      icon: getIconStatusPath(parseInt(get(information, "status", 99))),
      statusName: getStatusName(get(information, "status", 99)),
      dealer_name: get(information, "dealer_name", ""),
      delivery_date: get(information, "delivery_date", "-"),
      finance_contract_number: get(information, "finance_contract_number", "-"),
      model_name: get(information, "model_name", ""),
      next_service: get(information, "next_service", ""),
      phone: get(information, "phone", "-"),
      status: get(information, "status", ""),
      customer_name: get(information, "customer_name", "-"),
      engine_hour: get(information, "engine_hour", ""),
    };
            ...
            >
              <div style={{ flexDirection: "row", display: "flex" }}>
                <div className="b-r table-tan" style={{ minWidth: "auto" }}>
                  ...
                  <br />
                  <small style={{ color: "#B2BABB" }}>{info.gpsdate}</small>
                  <br />
                  ...
                  <table style={{ width: "100%" }}>
                    <thead>
                      ...
                      <tr>
                        ...
                        >
                          <small>
                            {t("my_vehicles_78")} : {info.model_code}
                          </small>
                        </th>
                        <th>
                          <small style={{ paddingRight: 5 }}>
                      ...
                      <tr>
                        ...
                        <th>
                          <small>
                            {t("realtime_157")} : {info.finance_contract_number}
                          </small>
                        </th>
                      </tr>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {/* <img alt="image" src={require('./icons/ic_explan.png')} /> */}
                              </button>
                            </div>
                            {loadingVideo && (
                              <div
                  ...
                  <table style={{ width: "100%" }}>
                    <tbody>
                      ...
                      <th className='border-right-sensor'> <small>{t("realtime_46")}</small></th> */}
                        ...
                        {/* {t("Sensor Dynamic")} */}
                        {dynamicSensor && this.setdynamicSensor(dynamicSensor)}
                      </tr>
                      ...
                      <tr>
                        ...
                        <th>
                          <span
                            style={{ fontSize: 12, margin: "0px 6px 0px 1px" }}
                          >
                            PTO
                          </span>
...
const mapStateToProps = (state) => ({
  ...
  hideFooter: state.controlroomdealer.hideFooter,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setHideFooter: () => dispatch(RealtimeNewActions.setHideFooter()),
