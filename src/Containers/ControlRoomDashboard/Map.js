...
const Map = (props) => {
  ...
  const initialVehiclesData = useSelector(
    (state) => state.controlroomdealer.initialVehiclesData
  );
  ...
  // const displayVehicle = useSelector(state => state.controlroom.displayVehicle);
  // const geofencesEnabled = useSelector(state => state.controlroom.geofencesEnabled);
  // const initialVehiclesData = useSelector(state => state.controlroom.initialVehiclesData);
  ...
  useEffect(() => {
    // console.log('initialVehiclesData', firstLoad, initialVehiclesData)
    // if (firstLoad) firstLoad = false
    if (!isEmpty(initialVehiclesData)) isFitBounds = true;
    // map
  }, [initialVehiclesData]);
  ...
  for (let index in initialVehiclesData) {
    let vid = initialVehiclesData[index].vid;
    let found = true;
    ...
    if (
      ...
      initialVehiclesData[index].vid == informationCR.vid
    ) {
      // console.log("ทำลบที่ตรงกับ info  : ", initialVehiclesData[index])
      found = false;
    }
    //#endregion

    if (
      initialVehiclesData[index].lat == 0 &&
      initialVehiclesData[index].lng == 0
    ) {
      // console.log("lat lng 0 0 : ", initialVehiclesData[index])
      found = false;
    }
    ...
    found &&
      data.push({
        ...
        location_type: "Force",
        location: {
          latitude: initialVehiclesData[index].lat,
          longitude: initialVehiclesData[index].lng,
          street: {
            id: initialVehiclesData[index].vid,
        ...
        month: "2019-10",
        image: {
          class_type: initialVehiclesData[index].vehicle_type_id,
          // "class_type": initialVehiclesData[index].class_type,
          status: initialVehiclesData[index].status,
          // "status": statusCar(initialVehiclesData[index]),
          course: initialVehiclesData[index].course,
        },
        vid: initialVehiclesData[index].vid,
        licenseplate: initialVehiclesData[index].licenseplate,
        vehicle_name: initialVehiclesData[index].vehicle_name,
        vin_no: initialVehiclesData[index].vin_no,
      });
  }
  //#endregion
  ...
  const setActiceMarker = (information) => {
    return (
      <MarkerMap
        ...
        lat={get(information, "lat")}
        lng={get(information, "lng")}
          ...
          {map !== null && (
            <>
              ...
              <MapControlsCustom
                ...
                infoHidden={true}
                hiddenButtons={["nameGeof", "geofences", "geofLicen"]}
                width={400}
              />
              ...
              >
                <div className="total-vehicle">
                  <center>
                    <span>
                      {initialVehiclesData
                        ? numberWithComma(initialVehiclesData.length)
                        : 0}
                    </span>{" "}
                    {t("summary_95")}
                  </center>
          ...
          {clusterDisable || clusterEnabled
            ...
            : points.map((cluster, index) => {
                ...
                return (
                  <MarkerMap
                    // key={cluster.id}
                    key={get(initialVehiclesData, "info.vid")}
                    lat={latitude}
                    lng={longitude}
                    onClick={() => {
                      getInformation(initialVehiclesData[index].vid);
                      dispatch(setVid(initialVehiclesData[index].vid));
                      // dispatch(getInformationMarker(cluster.vid, 'Marker'))
                      // _markerFocus = cluster.vid
                    }}
                    bottom={_tooltipButtom}
