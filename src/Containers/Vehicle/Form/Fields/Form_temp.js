...
const Form_temp = (props) => {
    ...
    const fetchData = () => {
        ...
        axios.all([requestOne, requestTwo]).then(axios.spread((...responses) => {
            ...
            let temp = dataSetting.find(item => item.id === 1)  //temp
            ...
            !isEmpty(get(temp, 'options.list', [])) && setInputFields(get(temp, 'options.list', []))
    ...
    const handleInputChange = (index, event) => {
        ...
        } else if (event.target.name === "max_val") {
            values[index].max_val = event.target.value;

        } else if (event.target.name === "min_val") {
            values[index].min_val = event.target.value;
    ...
    const handleSubmit = e => {
        ...
        axios
            .post(ENDPOINT_BASE_URL + 'fleet/setting/options/temperature', data, { headers })
                ...
                <Row>
                    {inputFields.map((inputField, index) => (
                        <Fragment key={`${inputField}~${index}`}>
                            ...
                            <Col md="2">
                                <div className="form-group">
                                    <label htmlFor="max_val">{t('vehicle_27')}<span className="text-danger">*</span> :</label>
                                    <input
                                        required
                                        type="text"
                                        className="form-control"
                            ...
                            <Col md="2">
                                <div className="form-group">
                                    <label htmlFor="min_val">{t('vehicle_28')}<span className="text-danger">*</span> :</label>
                                    <input
                                        required
                                        type="text"
                                        className="form-control"
