...
function cellRender(cellData) {
  // return <img src={cellData.data.iconUrl}/>
  return <img src={cellData.text} />;
  // return <img src={cellData.value}/>
}
...
class Table extends React.Component {
  ...
  async deleteProfileSetting() {
    ...
    if (is_default == 1) {
      Swal.fire(
        "ล้มเหลว",
        "ไม่สามารถลบได้ เพราะเป็นโปรไฟล์พื้นฐานของตารางที่สร้างโดยระบบ",
        "error"
      );
      return;
    }
    ...
    return (
      <div className="row">
        <div className="col-md-12">
          <div
            style={
              this.state.setting_profile.length == 0 ? { display: "none" } : {}
            }
            className="row"
          >
            <div className="col-lg-12 text-left">
              ...
              >
                ...
                <div className="form-group">
                  <button
                    style={{ marginLeft: 15, borderColor: "#1ab394" }}
                    onClick={() =>
                ...
                <div className="form-group">
                  <button
                    style={{ marginLeft: 15, borderColor: "red" }}
                    onClick={this.deleteProfileSetting}
                    className="btn btn-default"
                  >
                    Delete Setting
                  </button>
          ...
          <DataGrid
            ...
            dataSource={
              this.props.mode == "api" ? this.store : this.props.dataSource
            }
            showBorders={true}
            remoteOperations={this.props.mode == "api" ? true : false}
            onToolbarPreparing={this.onToolbarPreparing}
            onSelectionChanged={
              typeof this.props.selectedCallback !== "undefined"
          ...
          >
            {this.props.column &&
              this.props.column.map((element, i) => {
                return (
                  <Column
                    key={i}
                    dataField={element.column_name}
                    caption={element.column_caption}
                    cellRender={element.column_render === true && cellRender}
                  />
                );
              })}
            <Export
