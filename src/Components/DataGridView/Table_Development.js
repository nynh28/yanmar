...
class TableDevelopment extends React.Component {
  constructor(props) {
    super(props);
  ...
  async deleteProfileSetting() {
    Swal.fire({
      ...
      if (result.value) {
        ...
        var response = await fetch('https://tableprofile.onelink-iot.com/delete/setting', {
        ...
        if (responseJson.result == true) {
          Swal.fire('Success', 'Success to delete setting', 'success')
          this.load_setting_profile()
          return;
        }
        Swal.fire('Fail', 'Fail to delete setting please contact administrator', 'error')
        return;
      }
    })
  ...
  async saveProfileSetting() {
    ...
    var response = await fetch('https://tableprofile.onelink-iot.com/save/setting', {
      method: 'POST', // *GET, POST, PUT, DELETE, etc.
      headers: {
        "Accept": "text/html",
        "Content-Type": "application/json"
  ...
  async load_setting_profile(forceselected = false) {
    ...
    //var response = await fetch('https://tableprofile.onelink-iot.com/load/setting', {
    var response = await fetch('https://tableprofile.onelink-iot.com/load/setting', {
      method: 'POST', // *GET, POST, PUT, DELETE, etc.
      headers: {
        "Accept": "text/html",
  ...
  async changeProfile() {
    console.log(this.select_profile.current.value);
    this.setState({
  ...
  async loadState() {
    if (this.state.table_id == 0 || this.state.user_id == 0 || this.select_profile.current.value.split("|")[0] == "") {
      return {}
    }
    var array_value = this.select_profile.current.value.split("|");
    var table_id = array_value[0]
    ...
    console.log(object);
    var response = await fetch('https://tableprofile.onelink-iot.com/load/state', {
      method: 'POST', // *GET, POST, PUT, DELETE, etc.
      headers: {
        "Accept": "text/html",
        "Content-Type": "application/json"
  ...
  render() {
    ...
    console.log("searchPanel", this.props.searchPanel)
        ...
        <div className="col-md-12">
          <div style={this.state.setting_profile.length == 0 ? { display: 'none' } : {}} className="row">
            <div className="col-lg-12 text-left">
              ...
              <div className="form-inline float-right" style={{ marginBottom: 15 }}>
                ...
                <div className="form-group">
                  <button style={{ marginLeft: 15, }} onClick={() => window.jQuery('#setting-modal').modal('show')} className="btn btn-success">Add Setting </button>
                </div>
                <div className="form-group">
                  <button style={{ marginLeft: 15, }} onClick={this.deleteProfileSetting} className="btn btn-danger">Delete Setting</button>
                </div>
              </div>
            </div>
          ...
          >
            ...
            {this.props.column && this.props.column.map((element, index) => {
              return (
                <Column key={index} dataField={element.column_name} caption={element.column_caption} />
              )
            })}
          ...
          <button onClick={() => { console.log(JSON.stringify(this.datagrids.current.instance.state())) }} className="btn btn-info">State</button>
            ...
            <div className="modal-dialog" role="document">
              <div className="modal-content">
                <div className="modal-header">
                  ...
                  <h4 className="modal-title">Add Setting</h4>
                </div>
