    ...
    >
      {
        get(arg.editing, 'enabled', false) && <Column fixed={true} type="buttons" width={110}
          buttons={['edit', 'delete', arg.customButton]} />
      }
      ...
      {arg.column && arg.column.map((element, index) => {
        // console.log(element)
        return (
          <Column key={index} dataField={element.column_name} caption={t(element.column_caption)} dataType={element.dataType}
            cellRender={element.column_render} fixed={element.fixed} fixedPosition={"right"} calculateCellValue={element.calculateCellValue} visible={element.visible} />
          // customizeText={(data) => element.column_name.dataType == 'float' ? Number(data.value).toFixed(element.column_name.digit): data.value} />
        )
      })}
      ...
      <ColumnChooser enabled={false} mode="select" title={t("dg_select_item")} >
        {arg.column && arg.column.map((element, index) => {
          return (
            <Column
              key={index} dataField={element.column_name} caption={t(element.column_caption)} />
          )
        })}
      </ColumnChooser>
      <SearchPanel
      ...
      <GroupPanel emptyPanelText={t("my_vehicles_59")} visible={true} />
      ...
      this.setState({
        ...
        if (forceselected == true) {
          this.setState({
            ...
            // console.log(this.state.setting_selected);
            this.changeProfile()
          })
        }
      })
