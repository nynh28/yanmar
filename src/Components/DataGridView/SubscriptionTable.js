...
const DataGridTrans = (arg) => {
  ...
  const lookupDataSourceConfig = {
    store: {
      type: "array",
      data: [
        { id: "0", name: t("calendar_27") },
        { id: "-7", name: "7 วันก่อน" },
        { id: "-30", name: "30 วันก่อน" },
      ],
      key: "id",
    },
    pageSize: 10,
      ...
      {arg.column &&
        arg.column.map((element, index) => {
          return (
            ...
            >
              {element.column_name === "subscriptionDate" && (
                <Lookup
...
class Table extends React.Component {
      ...
      load: (loadOptions) => {
        ...
        [
          ...
          if (i in loadOptions && isNotEmpty(loadOptions[i])) {
            let prm = JSON.parse(JSON.stringify(loadOptions[i]));
            if (i === "filter") {
              if (prm.includes("and")) {
                prm.forEach((itm, index) => {
                  if (get(itm, "[0]") === "subscriptionDate")
                    itm[1] = "contains";
                  return itm;
                });
              } else if (get(prm, "[0]") === "subscriptionDate")
                prm[1] = "contains";
            }
            //#region
            // if (i === 'filter') {
            //   if (prm.includes('and')) {
            //     prm.forEach((itm, index) => {
            //       if (get(itm, '[1]') === "=") itm[1] = "contains"
            //       return itm
            //     });
            //   } else if (get(prm, '[1]') === "=") prm[1] = "contains"
            // }
            //#endregion
            params += `${i}=${encodeURIComponent(JSON.stringify(prm))}&`;
          }
        });
  ...
  componentWillUnmount() {
    ...
    if (this.datagrids.current && cookiesOptions && cookiesOptions.enable) {
      ...
      let config1 = JSON.stringify(this.datagrids.current.instance.state());
      this.props.setCookiesTableConfig(cookiesOptions.name, config1);
  ...
  calculateFilterExpression(value, selectedFilterOperations, target) {
    ...
    if (this.lookup) {
      console.log("xxxxxxx", this.lookup);
      //filter
    }
