...
const DataGridTrans = (arg) => {
  ...
  return (
    ...
    >
      ...
      <Summary>
        <TotalItem
          column={arg.columnCount || "customer_id"}
          summaryType="count"
...
class Table extends React.Component {
    ...
    this.store = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(queryUrl, {
          ...
          .then((data) => {
            return {
              data: data.result.customer_list,
  ...
  onExporting(e) {
    ...
    exportDataGrid({
      ...
      customizeCell: ({ gridCell, excelCell }) => {
        ...
        if (gridCell.rowType === "totalFooter") {
          if (footerSummary !== undefined) {
            ...
            if ((hideCount || deleteRow) && gridCell.value !== undefined)
      ...
      .then((cellRange) => {
        let lstColumnVisible = e.component
          .getVisibleColumns()
          .filter((item) => item.type !== "groupExpand");
          ...
          for (let i in list) {
            if (!isEmpty(list[i])) {
              ...
              if (typeof index_cell === "string") {
                let dataRow = lstColumnVisible.findIndex(
                  (item) => item.dataField === index_cell
                );
                if (dataRow >= 0)
                  index_cell = dataRow + get(topLeftSpace, "left", 1);
                else continue;
              }
              ...
              if (merge_cell !== undefined) {
                if (typeof merge_cell === "number")
                  worksheet.mergeCells(row, 1, row, merge_cell);
                else if (Array.isArray(merge_cell))
                  worksheet.mergeCells(row, merge_cell[0], row, merge_cell[1]);
                else
              ...
              if (font !== undefined)
                headerRow.getCell(index_cell || 1).font = font;
              if (alignment !== undefined)
                headerRow.getCell(index_cell || 1).alignment = alignment;
              if (fill !== undefined)
                headerRow.getCell(index_cell || 1).fill = fill;
              if (border !== undefined)
                headerRow.getCell(index_cell || 1).border = border;
            }
          }
        }
        if (footerCustom !== undefined) {
          ...
          for (let i in list) {
            if (!isEmpty(list[i])) {
              ...
              if (font !== undefined)
                footerRow.getCell(index_cell || 1).font = font;
              if (alignment !== undefined)
                footerRow.getCell(index_cell || 1).alignment = alignment;
              if (fill !== undefined)
                footerRow.getCell(index_cell || 1).fill = fill;
              if (border !== undefined)
                footerRow.getCell(index_cell || 1).border = border;
            }
          }
        }
      })
  ...
  componentWillUnmount() {
    ...
    if (this.datagrids.current && cookiesOptions && cookiesOptions.enable) {
      ...
      this.props.setCookiesTableConfig(
        cookiesOptions.name,
        config,
        this.state.setting_selected
      );
...
const mapDispatchToProps = (dispatch) => ({
  setCookiesTableConfig: (tableName, config, setting) =>
    dispatch(CommonActions.setCookiesTableConfig(tableName, config, setting)),
