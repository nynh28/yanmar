...
import { ENDPOINT_SETTING_BASE_URL, ENDPOINT_BASE_URL } from '../../Config/app-config';
...
const DataGridTrans = (arg) => {
  ...
  let customButton = [], selectionVisible = true, ExportEnable = true, GroupPanelVisible = true, zoomVisible = true, selectItemVisible = true
  if (arg.customButton) customButton = [...arg.customButton]
  if (arg.selectionVisible != undefined) selectionVisible = arg.selectionVisible
  if (arg.ExportEnable != undefined) ExportEnable = arg.ExportEnable
  if (arg.GroupPanelVisible != undefined) GroupPanelVisible = arg.GroupPanelVisible
  if (arg.zoomVisible != undefined) zoomVisible = arg.zoomVisible
  if (arg.selectItemVisible != undefined) selectItemVisible = arg.selectItemVisible
  ...
  return (
    <DataGrid
      ...
      onToolbarPreparing={(e) => {
        ...
        zoomVisible && e.toolbarOptions.items.unshift(
          {
            location: 'after',
            widget: 'dxButton',
            options: {
    ...
    >
      ...
      {arg.column && arg.column.map((element, index) => {
        return <Column
          key={index}
          cssClass={element.subTable && 'header-column-center'}
          dataField={element.column_name}
          caption={t(element.column_caption)}
          cellRender={element.column_render}
          groupCellTemplate={element.column_render ? (elem, data) => arg.groupCellTemplate(elem, data, element.column_render) : false}
          fixed={element.fixed}
          fixedPosition={element.fixedPosition || "right"}
          sortOrder={arg.sorting(index + 1)}
          calculateCellValue={element.calculateCellValue}
          // visible={false}>
          visible={arg.setVisibleColumn(element, index)}
        >
          {element.subTable && element.subTable.map((ele, idx) => {
            return <Column
              dataField={ele.column_name}
              caption={t(ele.column_caption)}
              cellRender={ele.column_render}
              groupCellTemplate={ele.column_render ? (elem, data) => arg.groupCellTemplate(elem, data, ele.column_render) : false}
              sortOrder={arg.sorting(idx + 1)}
              calculateCellValue={ele.calculateCellValue}
              // visible={false}>
              visible={arg.setVisibleColumn(ele, idx)}
            >
            </Column>
          })}
        </Column>
      ...
      <GroupPanel emptyPanelText={t("dg_drang_coumn")} visible={GroupPanelVisible} />
      <Scrolling mode={arg.scrolling || "standard"} />
...
class Table extends React.Component {
  ...
  groupCellTemplate(elem, data, func) {
    ...
    if (typeof displayValue === 'object') displayValue = this.replaceTranslationText(displayValue)
    ...
    if (groupContinuedMessage || groupContinuesMessage) {
      let textCinti = ' ('
      if (groupContinuedMessage) textCinti += this.replaceTranslationText(t("grid_9"))
  ...
  onExporting(e) {
    ...
    let { footerSummary, headerCustom, topLeftSpace } = this.props
    let topSpace = 1
    let leftSpace = 1
    ...
    exportDataGrid({
      ...
      customizeCell: ({ gridCell, excelCell }) => {
        ...
        let data = column.find((item) => item.column_name === gridCell.column.dataField)
        ...
        if (gridCell.rowType === 'group') {
          ...
          if (data && data.column_render && gridCell.value !== undefined) {
            ...
            if (typeof value === 'object') value = this.replaceTranslationText(value)
            let text = this.replaceTranslationText(t("dg_count"))
            let count = get(gridCell, 'groupSummaryItems[0].value')
            excelCell.value = gridCell.column.caption + ": " + value + " (" + text + ': ' + count + ')'
          }
        }
              ...
              if (typeof objSum[key] === 'object') {
                ...
                if (text !== undefined) {
                  ...
                  if (Array.isArray(text)) {
                    _text = text.map((data) => {
                      if (typeof data === 'object') return this.replaceTranslationText(data)
                      else return data
                    }).join('')
                  } else _text = text
      ...
      let lstColumnVisible = e.component.getVisibleColumns().filter((item) => item.type !== "groupExpand")
      // let lstColumnVisible = e.component.getVisibleColumns()
      // console.log('lstColumnVisible', lstColumnVisible)
      let { headerCustom, footerCustom, footerSummary } = this.props
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        let name = "Export", { exportName } = this.props
  ...
  componentWillUnmount() {
    ...
    // console.log('setting_selected', this.state.setting_selected, typeof this.state.setting_selected)
    ...
    if (this.datagrids.current && cookiesOptions && cookiesOptions.enable) {
      ...
      this.props.setCookiesTableConfig(cookiesOptions.name, config, this.state.setting_selected)
  ...
  calculateFilterExpression(value, selectedFilterOperations, target) {
    ...
    if (target === 'headerFilter' && value === 'asc') {
      ...
      this.filterValues.splice(this.filterValues.findIndex((e) => e == 'desc'), 1)
    }
    if (target === 'headerFilter' && value === 'desc') {
      this.filterValues.push(value)
      this.filterValues.splice(this.filterValues.findIndex((e) => e == 'asc'), 1)
    }
  ...
  async load_setting_profile(forceselected = false) {
    ...
    let { cookiesOptions, TbSettings } = this.props
    let setting_selected = TbSettings[get(cookiesOptions, 'name')]
    // console.log('setting_selected', setting_selected)
    ...
    var response = await fetch(ENDPOINT_SETTING_BASE_URL + 'fleet/setting/load/setting', {
      method: 'POST', // *GET, POST, PUT, DELETE, etc.
      headers: {
        "Accept": "text/html",
        "Content-Type": "application/json"
    ...
    if (responseJson.result == true) {
      ...
      this.setState({
        ...
        // console.log('6666666')
        if (setting_selected) {
          this.setState({ setting_selected }, () => {
            this.changeProfile()
          })
        } else if (forceselected == true) {
          this.setState({
  ...
  async loadState() {
    ...
    if (cookiesOptions && TbCookies[cookiesOptions.name] && this.firstLoad) {
      ...
      var response = await fetch(ENDPOINT_SETTING_BASE_URL + 'fleet/setting/load/state', {
        method: 'POST', // *GET, POST, PUT, DELETE, etc.
        headers: {
          "Accept": "text/html",
          "Content-Type": "application/json"
      ...
      if (responseJson.result == true) {
        ...
        return JSON.parse(JSON.stringify(responseJson.state))
      } else {
        return {}
      }
    }
  ...
  onOptionChanged = (e) => {
    if (e.fullName.split('.')[1] == "filterValues") {
      if (e.value != null) {
        if ((e.value.find(e => e == 'asc') == 'asc') && (this.state['' + e.fullName.split('[')[1][0]] != 'asc')) {
          sort['' + e.fullName.split('[')[1][0]] = 'asc'
        }
        else if ((e.value.find(e => e == 'desc') == 'desc') && (this.state['' + e.fullName.split('[')[1][0]] != 'desc')) {
          sort['' + e.fullName.split('[')[1][0]] = 'desc'
        }
      }
      else if (e.previousValue) {
        if ((e.previousValue.find(e => e == 'asc') == 'asc') && (this.state['' + e.fullName.split('[')[1][0]] != 'asc')) {
          sort['' + e.fullName.split('[')[1][0]] = ''
        }
        else if ((e.previousValue.find(e => e == 'desc') == 'desc') && (this.state['' + e.fullName.split('[')[1][0]] != 'desc')) {
          sort['' + e.fullName.split('[')[1][0]] = ''
        }
      }
    }
  ...
  onShowColumnChooser(e) {
    if (get(this.datagrids, 'current._instance')) {
      this.props.setValue("visibleColumnList", this.datagrids.current._instance.getVisibleColumns())
  ...
  onChangeColumnOption(value) {
    value.forEach((item) => {
      // console.log('current', this.datagrids.current)
      // console.log('columnOption', this.datagrids.current._instance.columnOption(item.column_name))
      if (this.datagrids.current._instance.columnOption(item.column_name) && this.datagrids.current._instance.columnOption(item.column_name).visible !== item.visible) {
        this.datagrids.current._instance.columnOption(item.column_name, "visible", item.visible);
      }
    })
        ...
        <div className="row">
          <div className="col-md-12">
              ...
              <div className="col-lg-12 text-left">
                {/* {!this.props.showHeaderLing && this.props.showHeaderLing != undefined ? "" : <hr></hr>} */}
                {
                  ...
                  <div className="form-inline float-right" style={{ marginBottom: 15 }}>
                    ...
                    <div className="form-group">
                      <button style={{ marginLeft: 15, marginTop: 5, width: 150 }} onClick={() => window.jQuery('#setting-modal').modal('show')} className="btn btn-success">{t("dg_add_setting")} </button>
                    </div>
                    ...
                    <div className="form-group">
                      ...
                      <button style={{ marginLeft: 15, marginTop: 5, width: 150 }} onClick={() => {
                        this.setAlertSetting(true, 3, "delete", '', false)
                      }} className="btn btn-danger">{t("dg_delete_setting")}</button>
                    </div>
                  </div>
                }
              </div>
            ...
            <DataGridTrans
              ...
              customButtonOnly={this.props.customButtonOnly}
              ...
              column={this.props.column}
              subColumn={this.props.subColumn}
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    <div className="form-group">
                      ...
                      <input type="text" className="form-control" ref={this.modal_profile_name} id="settingName" />
                    </div>
                  </div>
                  <div className="modal-footer">
                    <button type="button" className="btn btn-success" onClick={this.saveProfileSetting} >{t('submit')}</button>
                  </div>
                </div>
              </div>
            </div>
...
/* ----------------------- Format headerCustom and footerCustom -----------------------
  ...
  2.ใน object ของ list สามารถส่งค่าได้ดังนี้ text, index_cell, index_row, merge_cell, height, font, alignment, fill และ border
     2.1 text (type: string, array)
          => set ข้อความที่ต้องการให้โชว์ใน row/cell นั้น
  ...
  ***หากต้องการเว้นบรรทัดโดยไม่ต้องการ set (rowTotal กับ index_row) ให้ส่งเป็น object ว่าง({}) ใน row ที่ต้องการ Ex. [ {text:'Row 1'}, {}, {text:'Row 3'} ]***
...
/* ----------------------- Format footerSummary --------------------------------------
  1.ส่งค่ามาเป็น object โดยมี key เป็น hideCount, deleteRow และ objSum => { hideCount: boolean, objSum: {} }
     1.1 hideCount (type: boolean)
          => set ค่าเมื่อต้องการซ่อนการนับจำนวนของตาราง (default: false)
     1.2 deleteRow (type: boolean)
          => set ค่าเมื่อต้องการลบแถว footerSummary (default: false)
     1.3 objSum (type: object)
